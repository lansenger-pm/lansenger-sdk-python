...
藍信平台的框架無關 Python SDK——支援藍信應用、組織機械人及個人機械人。
...
```python
# appToken 由系統自動管理——只需設定 app_id + app_secret
client = LansengerClient(app_id="your-appid", app_secret="your-secret")

# 你也可以手動取得/撤銷 token
token = await client.get_token()
client.invalidate_token()  # 強制下次呼叫時更新
```
...
is_valid = verify_callback_signature(
    ...
    callback_token="回呼簽名令牌",  # 為空時回退到 encoding_key
)
```
...
client = LansengerClient(
    app_id="...", app_secret="...",
    encoding_key="BASE64_AES密鑰", callback_token="回呼簽名令牌",
...
`LansengerSyncClient` 提供所有相同方法，簽名一致（阻塞式）：
...
> **個人機械人**只能收發訊息和上載/下載檔案。無法訪問通訊錄、群組、日曆或 OAuth2。
>
...
除了身份類型，特定 API 呼叫還取決於藍信開發者中心的權限開關。組織可能限制開發者訪問，需要管理員協助。

**基礎權限（預設開啟）：**

| 權限 | 描述 |
|------|------|
| 獲取用戶基本資訊 | 獲取人員基本資訊用於系統/應用登入 |
...
遇到權限錯誤時，首先確認身份類型是否支援該操作，然後提示用戶在開發者中心開啟對應的高級權限（如無法訪問請聯絡組織管理員）。
...
├── src/lansenger_sdk/
│   ├── __init__.py          # 全部導出
│   ├── client.py            # LansengerClient（非同步）
│   ├── sync_client.py       # LansengerSyncClient（同步）
│   ├── config.py            # LansengerConfig
│   ├── auth.py              # TokenManager — appToken 生命週期管理
│   ├── oauth.py             # OAuth2 輔助
│   ├── constants.py         # API 端點、媒體類型、OAuth 範圍
│   ├── exceptions.py        # LansengerError 異常層級
│   ├── models.py            # 38+ dataclass 結果類型
